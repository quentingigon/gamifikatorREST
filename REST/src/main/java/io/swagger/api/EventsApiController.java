...
public class EventsApiController implements EventsApi {
    ...
    public ResponseEntity<Event> addEvent(@ApiParam(value = "New event" ,required=true )  @Valid @RequestBody Event body) {
        String accept = request.getHeader("Accept");
        if (accept != null && accept.contains("application/json")) {
            try {
                return new ResponseEntity<Event>(objectMapper.readValue("{  \"apiToken\" : \"apiToken\",  \"name\" : \"name\",  \"userId\" : \"userId\"}", Event.class), HttpStatus.NOT_IMPLEMENTED);
            } catch (IOException e) {
